...
class RepresentativeClusterMeanSampling(SamplingMethod):
  ...
  def select_batch_(self, model, N, already_selected, **kwargs):
    ...
    # Step 1: Create an index array for all indices
    all_indices = np.arange(len(self.y))

    # Step 2: Mask for indices not in already_selected
    not_in_already_selected_mask = ~np.isin(all_indices, already_selected_set)

    # Step 3: Mask for indices where any distance is less than min_margin_by_class
    below_margin_mask = np.any(distances < min_margin_by_class, axis=1)

    # Step 4: Combine the masks to get the final result
    unlabeled_in_margin = all_indices[not_in_already_selected_mask & below_margin_mask]    
    if len(unlabeled_in_margin) < N:
      print("Not enough points within margin of classifier, using simple uncertainty sampling")
      return rank_ind[0:N], min_margin
    ...
    mask = ~np.isin(rank_ind, selected_indices)
    remaining = rank_ind[mask]    
    selected_indices.extend(remaining[0:N-len(selected_indices)])
    return selected_indices, dist_to_centroid
