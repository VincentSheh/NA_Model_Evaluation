...
class BaseDeepAD(metaclass=ABCMeta):
    ...
    def save_model(self, path):
        ...
        torch.save({
            ...
            # Add other things if necessary, e.g., epoch, loss, etc.
        }, path)

    @classmethod
    def load_model(cls, path, device='cuda'):
        """Load model from a state dictionary."""
        model_instance = cls()  # Ensure this constructor properly initializes 'net'
        checkpoint = torch.load(path, map_location=device)
        
        # Check if 'net' is properly initialized
        if model_instance.net is None:
            raise ValueError("Model 'net' attribute is not initialized.")
        
        model_instance.net.load_state_dict(checkpoint['model_state_dict'])

        if 'optimizer_state_dict' in checkpoint and hasattr(model_instance, 'optimizer'):
            # Make sure the optimizer attribute is already set, possibly in the constructor.
            model_instance.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        
        # Handle other things if necessary, e.g., loaded epoch, loss, etc.
